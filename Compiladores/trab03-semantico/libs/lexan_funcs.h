...
_Bool isValidID(char *id){
	...
	if(isLetter(*tmp)){
		...
		if(!isKeyword(id)){
			return addNodeToken(&nodeList, "id", id);
		}
		return addNodeToken(&nodeList, id, id);
	}
	return false;
...
_Bool isTerminal(char c){
	...
	for(i = 0; i < MAXTERM; i++){
		if(c == terminals[i]){
			carac[0] = c;
			carac[1] = '\0';
			addNodeToken(&nodeList, carac, carac);
			return true;
		}
	}
	return false;
...
_Bool isConstantString(char *str){
	...
	if((count > 0 && count <= 256) && aspas == 2){
		addNodeToken(&nodeList, "conststr", str);
		return true;
	}
	return false;
}
...
_Bool isConstantNumber(char *num){
	...
	if((count > 0) && (count <= 10)){
		addNodeToken(&nodeList, "constnum", num);
		return true;	
	}
	return false;
}
...
_Bool isConstantChar(char *str){
	...
	if(((count < 2) && (count > 0)) && aspas == 2){
			addNodeToken(&nodeList, "CONSTCHAR", str);
			return true;
	}
	return false;
}
